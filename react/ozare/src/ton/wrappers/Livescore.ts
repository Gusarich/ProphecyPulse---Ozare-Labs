...
export const getAllMatchesByDate = async (date: string, category: string, timezone: number = 0 - 7) => {
  ...
  try {
    const resp = await (
      await fetch(`${liveScoreBaseUrl}/list-by-date?Category=${category}&Date=${date}&Timezone=${timezone}`, options)
    ).json();
...
export const getMatchDetails = async (eid: string, category: string) => {
  ...
  try {
    const resp = await (
      await fetch(`${liveScoreBaseUrl}/get-scoreboard?Category=${category}&Eid=${eid}`, options)
    ).json();
